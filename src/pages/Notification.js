...
export default function PostNotification() {
  ...
  // Mark a notification (verification) as read in Firestore
  const markVerificationAsRead = async (notifId) => {
    try {
      const schoolName = localStorage.getItem("schoolName");
      const user = auth.currentUser;
      if (!schoolName || !user) return;
      const notifDocRef = doc(db, "schools", schoolName, "users", user.uid, "notifications", notifId);
      await updateDoc(notifDocRef, { read: true });
      // This update will be reflected automatically by the onSnapshot listener
    } catch (err) {
  ...
  const handleClick = async (n) => {
    if (n.type === "comment") {
      ...
      // mark as read and optionally navigate to profile/timeline/etc.
      await markVerificationAsRead(n.id);
      // optionally navigate somewhere or just keep user on notifications page
    } else {
      // generic notification type: try to mark read if it belongs to userNotifs
      if (n.type && (n.id)) {
        await markVerificationAsRead(n.id);
      }
    }
